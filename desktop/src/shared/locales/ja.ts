...
export const ja: LocaleDict = {
  ...
  "settings.streamingCaptionsHint": "ディクテーション中は独立したストリーミングモデルが逐次草稿字幕を生成し、話し終えた後の確定テキストは現在選択中のモデルから出力されます。約 238MB のモデルのダウンロードが必要で、準備済みになるまで既存の字幕方式に自動で戻ります。内蔵オフライン認識のときのみ有効です。",
