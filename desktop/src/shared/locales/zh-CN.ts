...
export const zhCN = {
  ...
  "settings.streamingCaptions": "实时字幕（流式模型）",
  "settings.streamingCaptionsHint": "听写时由独立流式模型逐字生成草稿字幕，松手后仍用当前模型出终稿。需先下载约 238MB 的流式模型，未就绪时自动回退现有字幕方式。仅本地识别时生效。",
  "settings.streamingCaptionsModel": "流式字幕模型",
  "settings.streamingCaptionsNotReady": "开关已开但模型未就绪：听写暂用现有字幕方式（部分模型无实时字幕）。",
  ...
  "settings.autoUpdateCheckHint":
    "启动后向 GitHub 查询一次最新版本号（只请求版本号，不上传任何你的信息）。关闭后完全离线，仍可在关于页手动检查。",
  ...
  "settings.about.checkFailed": "检查更新失败：{{reason}}",
  "settings.about.checkFailedNetwork": "无法连接 GitHub",
  "settings.about.checkFailedRateLimit": "GitHub 接口访问过于频繁，请稍后再试",
  "settings.about.checkFailedServer": "GitHub 返回了错误",
  "settings.about.checkFailedOther": "返回内容异常",
  "settings.about.autoCheckOffHint": "自动检查更新已关闭——在你点「检查更新」之前不会发出任何请求。",
  ...
  "settings.about.privacyDesc":
    "所有配置（热键、人设、词典、密钥）都保存在本机 %APPDATA%\\SpeakType 下；历史记录仅存本地，可随时清空。除你自己配置的识别/润色服务外，SpeakType 唯一会自行发起的联网是可关闭的版本检查（设置 → 通用 → 自动检查更新，只向 GitHub 请求版本号）。",
